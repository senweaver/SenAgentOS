...
//!
//! 1. `SenAgentOS_AARDVARK_LIB` environment variable (full path to `aardvark.so`)
//! 2. `<workspace>/firmware/aardvark-sys/vendor/aardvark.so` (development default)
...
//! # Safety
//!
//! This crate is the **only** place in senagent where `unsafe` is permitted.
    ...
    AARDVARK_LIB
        .get_or_init(|| {
            let candidates: Vec<PathBuf> = vec![
                ...
                std::env::var("SenAgentOS_AARDVARK_LIB")
                    .ok()
                    .map(PathBuf::from)
                    .unwrap_or_default(),
            ...
            if !tried_any {
                eprintln!("[aardvark-sys] no library candidates found; set SenAgentOS_AARDVARK_LIB or place aardvark.so next to the binary");
            }
            None
        })
        .as_ref()
...
pub enum AardvarkError {
    ...
    #[error("aardvark.so not found -?set SenAgentOS_AARDVARK_LIB or place it next to the binary")]
    LibraryNotFound,
}
...
impl AardvarkHandle {
    ...
    ///
    /// Probes `0x08-?x77` with a 1-byte read; returns addresses that ACK.
