...
impl EmoteTool {
    ...
    async fn set_expression(&self, expr: Expression) -> Result<()> {
        ...
        let led_fifo = PathBuf::from("/tmp/SenAgentOS_led.fifo");
        if led_fifo.exists() {
        ...
        let output = tokio::process::Command::new("SenAgentOS-led")
    ...
    async fn play_emotion_sound(&self, emotion: &str) -> Result<()> {
        let sound_file = self.sounds.join(format!("{}.wav", emotion));
