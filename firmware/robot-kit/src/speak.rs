...
use crate::paths::{sounds_dir, tts_cache_dir, voice_models_dir};
...
impl SpeakTool {
    ...
    async fn play_sound(&self, sound: &str) -> Result<()> {
        let sound_dir = sounds_dir();

        let sound_file = sound_dir.join(format!("{}.wav", sound));
