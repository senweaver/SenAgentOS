...
//! - GPIO: Direct PWM control via rppal (enable `gpio` feature)
...
trait DriveBackend: Send + Sync {
    ...
    async fn get_odometry(&self) -> Result<(f64, f64, f64)>; // x, y, theta
}
...
/// GPIO backend — direct PWM motor control via rppal (Raspberry Pi GPIO).
///
/// Enable the `gpio` feature in Cargo.toml to activate this backend.
#[cfg(feature = "gpio")]
struct GpioDrive;

#[cfg(feature = "gpio")]
#[async_trait]
impl DriveBackend for GpioDrive {
    async fn move_robot(
        &self,
        linear_x: f64,
        linear_y: f64,
        ...
        // Omni-wheel drive: differential steering
        // linear_x = forward(+1)/back(-1), angular_z = rotate cw(+1)/ccw(-1)
        let left_speed = (linear_x - angular_z).clamp(-1.0, 1.0);
        let right_speed = (linear_x + angular_z).clamp(-1.0, 1.0);

        left_pwm.set_duty_cycle(left_speed.abs())?;
        right_pwm.set_duty_cycle(right_speed.abs())?;
...
/// Stub for GPIO backend when the `gpio` feature is disabled.
...
impl DriveTool {
    pub fn new(config: RobotConfig) -> Self {
        let backend: Arc<dyn DriveBackend> = match config.drive.backend.as_str() {
            ...
            #[cfg(feature = "gpio")]
            "gpio" => Arc::new(GpioDrive::new()),
            #[cfg(not(feature = "gpio"))]
            "gpio" => {
                tracing::warn!("GPIO backend requires the 'gpio' feature. Falling back to mock.");
                Arc::new(MockDrive)
            }
            _ => Arc::new(MockDrive),
        };
