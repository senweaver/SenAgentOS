...
export default function AgentChat() {
  ...
  // Guard against StrictMode double-mount: the first mount's cleanup closes the socket
  // (code 1006) before the second mount's connect() can succeed. Track whether a
  // successful onopen has ever occurred so we suppress false errors from this cycle.
  ...
  useEffect(() => {
    ...
    ws.onClose = (ev: CloseEvent) => {
      setConnected(false);
      // Suppress error for abnormal close codes that occur during StrictMode double-mount.
      // In StrictMode: connect → unmount (cleanup closes with 1006) → remount.
      // Only surface the error if we had a successful connection before.
      if (ev.code !== 1000 && ev.code !== 1001 && everConnected.current) {
        setError(`连接意外关闭 (code: ${ev.code})。请检查配置。`);
      }
    };

    ws.onError = () => {
      // Only surface error if a connection was previously established.
      // During StrictMode double-mount, the first connect() attempt may trigger onerror
      // from the previous socket's cleanup; suppress it.
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center animate-fade-in" style={{ color: 'var(--pc-text-muted)' }}>
            ...
            <p className="text-lg font-semibold mb-1" style={{ color: 'var(--pc-text-primary)' }}>SenAgent Agent</p>
