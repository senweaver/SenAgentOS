...
> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error('[SenAgentOS] Render error:', error, info.componentStack);
  }
...
function PairingDialog({ onPair }: { onPair: (code: string) => Promise<void> }) {
  ...
  // Fetch the current pairing code from the admin endpoint
  useEffect(() => {
    let cancelled = false;
    getAdminPairCode()
      .then((data) => {
      ...
      .catch(() => {
        // Admin endpoint not reachable ? user must check terminal
      })
      .finally(() => {
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('senagent-unauthorized', handler);
    return () => window.removeEventListener('senagent-unauthorized', handler);
  }, [logout]);
