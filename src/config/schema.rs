...
/// Top-level SenAgentOS configuration, loaded from `config.toml`.
///
/// Resolution order: `SAGENT_WORKSPACE` env → `active_workspace.toml` marker → `~/.senagent/config.toml`.
...
pub struct Config {
    ...
    /// API key for the selected provider. Overridden by `SAGENT_API_KEY` or `API_KEY` env vars.
    ...
    ///
    /// Can also be set via `SAGENT_EXTRA_HEADERS` environment variable using
    ...
    ///
    /// If omitted or empty, the locale is auto-detected from `SAGENT_LOCALE`,
...
pub struct SkillsConfig {
    ...
    /// Skill names excluded from the agent (dashboard/API). Matching is by manifest `name` (exact).
    #[serde(default)]
...
/// execution events back to SenAgentOS's gateway, updating a Slack message
...
pub enum ProxyScope {
    ...
    Environment,
    /// Apply proxy to all SenAgentOS-managed HTTP traffic (default).
    #[default]
    Internal,
...
fn parse_proxy_scope(raw: &str) -> Option<ProxyScope> {
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        "senagent" | "internal" | "core" => Some(ProxyScope::Internal),
...
pub struct QdrantConfig {
    ...
    /// Falls back to `QDRANT_COLLECTION` env var, or default "senagent_memories".
...
pub struct ObservabilityConfig {
    ...
    /// Service name reported to the OTel collector. Defaults to "senagent".
    #[serde(default)]
...
pub struct MatrixConfig {
    ...
    /// When set, SenAgentOS recovers room keys and cross-signing secrets on startup.
    #[serde(default)]
...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `SAGENT_WHATSAPP_APP_SECRET` environment variable
    /// Only used in Cloud API mode
    #[serde(default)]
    ...
    /// Example: `["@?SenAgentOS", "\\+?15555550123"]`
    #[serde(default)]
...
pub struct NextcloudTalkConfig {
    ...
    ///
    /// Can also be set via `SAGENT_NEXTCLOUD_TALK_WEBHOOK_SECRET`.
    #[serde(default)]
    ...
    #[serde(default)]
    pub proxy_url: Option<String>,
    /// Display name of the bot in Nextcloud Talk (e.g. "senagent").
...
/// How SenAgentOS receives events from Feishu / Lark.
///
...
pub struct WebAuthnConfig {
    ...
    /// Relying Party display name. Default: "SenAgentOS".
...
fn default_estop_state_file() -> String {
    "~/.senagent/estop-state.json".to_string()
}
...
///
/// When `enabled` is true, SenAgentOS validates incoming requests against a Nevis
...
pub struct NevisConfig {
    ...
    /// Nevis role to SenAgentOS permission mappings.
    #[serde(default)]
...
/// Maps a Nevis role to SenAgentOS tool permissions and workspace access.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(deny_unknown_fields)]
pub struct NevisRoleMappingConfig {
    ...
    #[serde(default)]
    pub senagent_permissions: Vec<String>,
...
pub struct AuditConfig {
    ...
    /// Path to audit log file (relative to senagent dir)
...
fn default_report_output_dir() -> String {
    "~/.senagent/security-reports".into()
}
...
impl Default for Config {
    fn default() -> Self {
        ...
        let senagent_dir = home.join(".senagent");

        Self {
            workspace_dir: senagent_dir.join("workspace"),
            config_path: senagent_dir.join("config.toml"),
            api_key: None,
            api_url: None,
            api_path: None,
...
fn default_config_dir() -> Result<PathBuf> {
    if let Ok(home) = std::env::var("HOME") {
        if !home.is_empty() {
            return Ok(PathBuf::from(home).join(".senagent"));
        }
    }
...
pub(crate) fn resolve_config_dir_for_workspace(workspace_dir: &Path) -> (PathBuf, PathBuf) {
    ...
    let legacy_config_dir = workspace_dir
        .parent()
        .map(|parent| parent.join(".senagent"));
...
/// `SAGENT_CONFIG_DIR` > `SAGENT_WORKSPACE` > active workspace marker > defaults.
pub async fn resolve_runtime_dirs_for_onboarding() -> Result<(PathBuf, PathBuf)> {
    let (default_senagent_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
    let (config_dir, workspace_dir, _) =
        resolve_runtime_config_dirs(&default_senagent_dir, &default_workspace_dir).await?;
...
impl ConfigResolutionSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::EnvConfigDir => "SAGENT_CONFIG_DIR",
            Self::EnvWorkspace => "SAGENT_WORKSPACE",
...
async fn resolve_runtime_config_dirs(
    default_senagent_dir: &Path,
    default_workspace_dir: &Path,
) -> Result<(PathBuf, PathBuf, ConfigResolutionSource)> {
    if let Ok(custom_config_dir) = std::env::var("SAGENT_CONFIG_DIR") {
        let custom_config_dir = custom_config_dir.trim();
        if !custom_config_dir.is_empty() {
            let senagent_dir = expand_tilde_path(custom_config_dir);
            return Ok((
                senagent_dir.clone(),
                senagent_dir.join("workspace"),
                ConfigResolutionSource::EnvConfigDir,
            ));
        }
    }

    if let Ok(custom_workspace) = std::env::var("SAGENT_WORKSPACE") {
        if !custom_workspace.is_empty() {
            let expanded = expand_tilde_path(&custom_workspace);
            let (senagent_dir, workspace_dir) = resolve_config_dir_for_workspace(&expanded);
            return Ok((
                senagent_dir,
                workspace_dir,
    ...
    if let Some((senagent_dir, workspace_dir)) =
        load_persisted_workspace_dirs(default_senagent_dir).await?
    {
        return Ok((
            senagent_dir,
            workspace_dir,
    ...
    Ok((
        default_senagent_dir.to_path_buf(),
...
fn config_dir_creation_error(path: &Path) -> String {
    format!(
        "Failed to create config directory: {}. If running as an OpenRC service, \
         ensure this path is writable by user 'senagent'.",
        path.display()
    )
}
...
fn has_ollama_cloud_credential(config_api_key: Option<&str>) -> bool {
    ...
    ["OLLAMA_API_KEY", "SAGENT_API_KEY", "API_KEY"]
        .iter()
        .any(|name| {
            std::env::var(name)
                .ok()
...
/// Parse the `SAGENT_EXTRA_HEADERS` environment variable value.
///
...
pub fn parse_extra_headers_env(raw: &str) -> Vec<(String, String)> {
    ...
    for entry in raw.split(',') {
        ...
        if let Some((key, value)) = entry.split_once(':') {
            ...
            if key.is_empty() {
                tracing::warn!("Ignoring extra header with empty name in SAGENT_EXTRA_HEADERS");
                continue;
            }
...
///
/// When the workspace is created outside of `senagent onboard` (e.g., non-tty
    ...
    let defaults: &[(&str, &str)] = &[
        (
            ...
            "# IDENTITY.md — Who Am I?\n\n\
             I am SenAgentOS, an autonomous AI agent.\n\n\
             ## Traits\n\
        ...
        (
            "SOUL.md",
            "# SOUL.md — Who You Are\n\n\
             You are SenAgentOS, an autonomous AI agent.\n\n\
             ## Core Principles\n\
...
impl Config {
    pub async fn load_or_init() -> Result<Self> {
        ...
        let (senagent_dir, workspace_dir, resolution_source) =
            resolve_runtime_config_dirs(&default_senagent_dir, &default_workspace_dir).await?;

        let config_path = senagent_dir.join("config.toml");

        fs::create_dir_all(&senagent_dir)
            .await
            .with_context(|| config_dir_creation_error(&senagent_dir))?;
        ...
        if config_path.exists() {
            ...
            let store = crate::security::SecretStore::new(&senagent_dir, config.secrets.encrypt);
    ...
    pub fn apply_env_overrides(&mut self) {
        // API Key: SAGENT_API_KEY or API_KEY (generic)
        if let Ok(key) = std::env::var("SAGENT_API_KEY").or_else(|_| std::env::var("API_KEY")) {
            if !key.is_empty() {
        ...
        // API Key & URL: SENWEAVER_API_KEY / SENWEAVER_BASE_URL for SenWeaver provider.
        if self
            .default_provider
            .as_deref()
            .is_some_and(|p| p.eq_ignore_ascii_case("senweaver"))
        {
        ...
        // 1) SAGENT_PROVIDER always wins when set.
        // 2) SAGENT_MODEL_PROVIDER/MODEL_PROVIDER (Codex app-server style).
        // 3) Legacy PROVIDER is honored only when config still uses default provider.
        if let Ok(provider) = std::env::var("SAGENT_PROVIDER") {
            if !provider.is_empty() {
                self.default_provider = Some(provider);
            }
        } else if let Ok(provider) =
            std::env::var("SAGENT_MODEL_PROVIDER").or_else(|_| std::env::var("MODEL_PROVIDER"))
        {
            if !provider.is_empty() {
                self.default_provider = Some(provider);
            }
        ...
        // Model: SAGENT_MODEL or MODEL
        if let Ok(model) = std::env::var("SAGENT_MODEL").or_else(|_| std::env::var("MODEL")) {
            if !model.is_empty() {
                self.default_model = Some(model);
            }
        }

        // Provider HTTP timeout: SAGENT_PROVIDER_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("SAGENT_PROVIDER_TIMEOUT_SECS") {
        ...
        // Extra provider headers: SAGENT_EXTRA_HEADERS
        // Format: "Key:Value,Key2:Value2"
        // Env var headers override config file headers with the same name.
        if let Ok(raw) = std::env::var("SAGENT_EXTRA_HEADERS") {
        ...
        // Workspace directory: SAGENT_WORKSPACE
        if let Ok(workspace) = std::env::var("SAGENT_WORKSPACE") {
        ...
        // Open-skills opt-in flag: SAGENT_OPEN_SKILLS_ENABLED
        if let Ok(flag) = std::env::var("SAGENT_OPEN_SKILLS_ENABLED") {
            if !flag.trim().is_empty() {
                match flag.trim().to_ascii_lowercase().as_str() {
                    "1" | "true" | "yes" | "on" => self.skills.open_skills_enabled = true,
                    "0" | "false" | "no" | "off" => self.skills.open_skills_enabled = false,
                    _ => tracing::warn!(
                        "Ignoring invalid SAGENT_OPEN_SKILLS_ENABLED (valid: 1|0|true|false|yes|no|on|off)"
                    ),
                }
            }
        }

        // Open-skills directory override: SAGENT_OPEN_SKILLS_DIR
        if let Ok(path) = std::env::var("SAGENT_OPEN_SKILLS_DIR") {
            let trimmed = path.trim();
            if !trimmed.is_empty() {
        ...
        if let Ok(flag) = std::env::var("SAGENT_SKILLS_ALLOW_SCRIPTS") {
            if !flag.trim().is_empty() {
                match flag.trim().to_ascii_lowercase().as_str() {
                    ...
                    _ => tracing::warn!(
                        "Ignoring invalid SAGENT_SKILLS_ALLOW_SCRIPTS (valid: 1|0|true|false|yes|no|on|off)"
                    ),
                }
            }
        }

        // Skills prompt mode override: SAGENT_SKILLS_PROMPT_MODE
        if let Ok(mode) = std::env::var("SAGENT_SKILLS_PROMPT_MODE") {
            if !mode.trim().is_empty() {
                if let Some(parsed) = parse_skills_prompt_injection_mode(&mode) {
                    self.skills.prompt_injection_mode = parsed;
                } else {
                    tracing::warn!(
                        "Ignoring invalid SAGENT_SKILLS_PROMPT_MODE (valid: full|compact)"
                    );
                }
            }
        }

        // Gateway port: SAGENT_GATEWAY_PORT or PORT
        if let Ok(port_str) =
            std::env::var("SAGENT_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
        {
        ...
        // Allow public bind: SAGENT_ALLOW_PUBLIC_BIND
        if let Ok(val) = std::env::var("SAGENT_ALLOW_PUBLIC_BIND") {
            self.gateway.allow_public_bind = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Require pairing: SAGENT_REQUIRE_PAIRING
        if let Ok(val) = std::env::var("SAGENT_REQUIRE_PAIRING") {
            self.gateway.require_pairing = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Temperature: SAGENT_TEMPERATURE
        if let Ok(temp_str) = std::env::var("SAGENT_TEMPERATURE") {
            match temp_str.parse::<f64>() {
                ...
                Ok(temp) => {
                    tracing::warn!(
                        "Ignoring SAGENT_TEMPERATURE={temp}: \
        ...
        // Reasoning override: SAGENT_REASONING_ENABLED or REASONING_ENABLED
        if let Ok(flag) = std::env::var("SAGENT_REASONING_ENABLED")
        ...
        if let Ok(raw) = std::env::var("SAGENT_REASONING_EFFORT")
            .or_else(|_| std::env::var("REASONING_EFFORT"))
            .or_else(|_| std::env::var("SAGENT_CODEX_REASONING_EFFORT"))
        {
        ...
        // Web search enabled: SAGENT_WEB_SEARCH_ENABLED or WEB_SEARCH_ENABLED
        if let Ok(enabled) = std::env::var("SAGENT_WEB_SEARCH_ENABLED")
            .or_else(|_| std::env::var("WEB_SEARCH_ENABLED"))
        {
            self.web_search.enabled = enabled == "1" || enabled.eq_ignore_ascii_case("true");
        }

        // Web search provider: SAGENT_WEB_SEARCH_PROVIDER or WEB_SEARCH_PROVIDER
        if let Ok(provider) = std::env::var("SAGENT_WEB_SEARCH_PROVIDER")
        ...
        // Brave API key: SAGENT_BRAVE_API_KEY or BRAVE_API_KEY
        if let Ok(api_key) =
            std::env::var("SAGENT_BRAVE_API_KEY").or_else(|_| std::env::var("BRAVE_API_KEY"))
        {
        ...
        // SearXNG instance URL: SAGENT_SEARXNG_INSTANCE_URL or SEARXNG_INSTANCE_URL
        if let Ok(instance_url) = std::env::var("SAGENT_SEARXNG_INSTANCE_URL")
        ...
        // Web search max results: SAGENT_WEB_SEARCH_MAX_RESULTS or WEB_SEARCH_MAX_RESULTS
        if let Ok(max_results) = std::env::var("SAGENT_WEB_SEARCH_MAX_RESULTS")
        ...
        // Web search timeout: SAGENT_WEB_SEARCH_TIMEOUT_SECS or WEB_SEARCH_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("SAGENT_WEB_SEARCH_TIMEOUT_SECS")
        ...
        // Storage provider key (optional backend override): SAGENT_STORAGE_PROVIDER
        if let Ok(provider) = std::env::var("SAGENT_STORAGE_PROVIDER") {
            let provider = provider.trim();
            if !provider.is_empty() {
        ...
        // Storage connection URL (for remote backends): SAGENT_STORAGE_DB_URL
        if let Ok(db_url) = std::env::var("SAGENT_STORAGE_DB_URL") {
        ...
        // Storage connect timeout: SAGENT_STORAGE_CONNECT_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("SAGENT_STORAGE_CONNECT_TIMEOUT_SECS") {
            if let Ok(timeout_secs) = timeout_secs.parse::<u64>() {
                if timeout_secs > 0 {
        ...
        // Proxy enabled flag: SAGENT_PROXY_ENABLED
        let explicit_proxy_enabled = std::env::var("SAGENT_PROXY_ENABLED")
            .ok()
            .as_deref()
        ...
        // Proxy URLs: SAGENT_* wins, then generic *PROXY vars.
        let mut proxy_url_overridden = false;
        if let Ok(proxy_url) =
            std::env::var("SAGENT_HTTP_PROXY").or_else(|_| std::env::var("HTTP_PROXY"))
        {
            self.proxy.http_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("SAGENT_HTTPS_PROXY").or_else(|_| std::env::var("HTTPS_PROXY"))
        {
            self.proxy.https_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("SAGENT_ALL_PROXY").or_else(|_| std::env::var("ALL_PROXY"))
        {
            self.proxy.all_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(no_proxy) =
            std::env::var("SAGENT_NO_PROXY").or_else(|_| std::env::var("NO_PROXY"))
        {
        ...
        if let Ok(scope_raw) = std::env::var("SAGENT_PROXY_SCOPE") {
            if let Some(scope) = parse_proxy_scope(&scope_raw) {
                self.proxy.scope = scope;
            } else {
                tracing::warn!(
                    scope = %scope_raw,
                    "Ignoring invalid SAGENT_PROXY_SCOPE (valid: environment|senagent|services)"
                );
            }
        }

        if let Ok(services_raw) = std::env::var("SAGENT_PROXY_SERVICES") {
    ...
    async fn resolve_config_path_for_save(&self) -> Result<PathBuf> {
        ...
        let (default_senagent_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
        let (senagent_dir, _workspace_dir, source) =
            resolve_runtime_config_dirs(&default_senagent_dir, &default_workspace_dir).await?;
        let file_name = self
            .config_path
            .file_name()
        ...
        let resolved = senagent_dir.join(file_name);
        tracing::warn!(
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        let senagent_dir = config_path
            .parent()
            .context("Config path must have a parent directory")?;
        let store = crate::security::SecretStore::new(senagent_dir, self.secrets.encrypt);
...
mod tests {
    ...
    async fn expand_tilde_path_expands_tilde_when_home_set() {
        ...
        let path = expand_tilde_path("~/.senagent");
    ...
    async fn config_dir_creation_error_mentions_openrc_and_path() {
        let msg = config_dir_creation_error(Path::new("/etc/senagent"));
        assert!(msg.contains("/etc/senagent"));
        assert!(msg.contains("OpenRC"));
        assert!(msg.contains("senagent"));
    }
...
"#;
    ...
    async fn parse_extra_headers_env_basic() {
        let headers = parse_extra_headers_env("User-Agent:MyApp/1.0,X-Title:senagent");
        assert_eq!(headers.len(), 2);
        assert_eq!(
            headers[0],
            ("User-Agent".to_string(), "MyApp/1.0".to_string())
        );
        assert_eq!(headers[1], ("X-Title".to_string(), "senagent".to_string()));
    }
    ...
    async fn parse_extra_headers_env_with_url_value() {
        ...
        assert_eq!(headers[0].1, "https://github.com/senweaver/senagent");
    }
    ...
    async fn parse_extra_headers_env_whitespace_trimming() {
        let headers = parse_extra_headers_env("  X-Title : senagent , User-Agent : cli/1.0 ");
        assert_eq!(headers.len(), 2);
        assert_eq!(headers[0], ("X-Title".to_string(), "senagent".to_string()));
        assert_eq!(
            headers[1],
    ...
    async fn parse_extra_headers_env_trailing_comma() {
        let headers = parse_extra_headers_env("X-Title:senagent,");
        assert_eq!(headers.len(), 1);
        assert_eq!(headers[0], ("X-Title".to_string(), "senagent".to_string()));
    }
...
"#;
        ...
        assert_eq!(parsed.extra_headers.get("X-Title").unwrap(), "senagent");
    }
...
"#;
    ...
    async fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("senagent_test_config");
        let _ = fs::remove_dir_all(&dir).await;
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_encrypts_nested_credentials() {
        let dir = std::env::temp_dir().join(format!(
            "senagent_test_nested_credentials_{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_atomic_cleanup() {
        let dir =
            std::env::temp_dir().join(format!("senagent_test_config_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).await.unwrap();
...
"#;
    ...
    async fn whatsapp_config_backend_type_cloud_precedence_when_ambiguous() {
        let wc = WhatsAppConfig {
            ...
            verify_token: Some("ver".into()),
            app_secret: None,
            session_path: Some("~/.senagent/state/whatsapp-web/session.db".into()),
            pair_phone: None,
            pair_code: None,
            allowed_numbers: vec!["+1".into()],
            mode: WhatsAppWebMode::default(),
    ...
    async fn checklist_gateway_serde_roundtrip() {
        let g = GatewayConfig {
            ...
            path_prefix: Some("/senagent".into()),
        ...
        assert_eq!(parsed.path_prefix.as_deref(), Some("/senagent"));
...
"#;
    ...
    fn clear_proxy_env_test_vars() {
        for key in [
            "SAGENT_PROXY_ENABLED",
            "SAGENT_HTTP_PROXY",
            "SAGENT_HTTPS_PROXY",
            "SAGENT_ALL_PROXY",
            "SAGENT_NO_PROXY",
            "SAGENT_PROXY_SCOPE",
            "SAGENT_PROXY_SERVICES",
            "HTTP_PROXY",
            "HTTPS_PROXY",
            "ALL_PROXY",
            "NO_PROXY",
    ...
    async fn env_override_api_key() {
        ...
        crate::util::set_env_var("SAGENT_API_KEY", "sk-test-env-key");
        config.apply_env_overrides();
        assert_eq!(config.api_key.as_deref(), Some("sk-test-env-key"));

        crate::util::remove_env_var("SAGENT_API_KEY");
    }
    ...
    async fn env_override_provider() {
        ...
        crate::util::set_env_var("SAGENT_PROVIDER", "anthropic");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("anthropic"));

        crate::util::remove_env_var("SAGENT_PROVIDER");
    }
    ...
    async fn env_override_model_provider_alias() {
        ...
        crate::util::remove_env_var("SAGENT_PROVIDER");
        crate::util::set_env_var("SAGENT_MODEL_PROVIDER", "openai-codex");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("openai-codex"));

        crate::util::remove_env_var("SAGENT_MODEL_PROVIDER");
    }
...
"#;
    ...
    async fn env_override_open_skills_enabled_and_dir() {
        ...
        crate::util::set_env_var("SAGENT_OPEN_SKILLS_ENABLED", "true");
        crate::util::set_env_var("SAGENT_OPEN_SKILLS_DIR", "/tmp/open-skills");
        crate::util::set_env_var("SAGENT_SKILLS_ALLOW_SCRIPTS", "yes");
        crate::util::set_env_var("SAGENT_SKILLS_PROMPT_MODE", "compact");
        config.apply_env_overrides();
        ...
        crate::util::remove_env_var("SAGENT_OPEN_SKILLS_ENABLED");
        crate::util::remove_env_var("SAGENT_OPEN_SKILLS_DIR");
        crate::util::remove_env_var("SAGENT_SKILLS_ALLOW_SCRIPTS");
        crate::util::remove_env_var("SAGENT_SKILLS_PROMPT_MODE");
    }
    ...
    async fn env_override_open_skills_enabled_invalid_value_keeps_existing_value() {
        ...
        crate::util::set_env_var("SAGENT_OPEN_SKILLS_ENABLED", "maybe");
        crate::util::set_env_var("SAGENT_SKILLS_ALLOW_SCRIPTS", "maybe");
        crate::util::set_env_var("SAGENT_SKILLS_PROMPT_MODE", "invalid");
        config.apply_env_overrides();
    ...
    async fn env_override_zero_claw_provider_overrides_non_default_provider() {
        ...
        crate::util::set_env_var("SAGENT_PROVIDER", "openrouter");
    ...
    async fn env_override_model() {
        ...
        crate::util::set_env_var("SAGENT_MODEL", "gpt-4o");
        config.apply_env_overrides();
        assert_eq!(config.default_model.as_deref(), Some("gpt-4o"));

        crate::util::remove_env_var("SAGENT_MODEL");
    }
    ...
    async fn save_repairs_bare_config_filename_using_runtime_resolution() {
        let _env_guard = env_override_lock().await;
        let temp_home =
            std::env::temp_dir().join(format!("senagent_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let resolved_config_path = temp_home.join(".senagent").join("config.toml");

        let original_home = std::env::var("HOME").ok();
        crate::util::set_env_var("HOME", &temp_home);
        crate::util::set_env_var("SAGENT_WORKSPACE", &workspace_dir);
        ...
        crate::util::remove_env_var("SAGENT_WORKSPACE");
    ...
    async fn env_override_workspace() {
        ...
        crate::util::set_env_var("SAGENT_WORKSPACE", "/custom/workspace");
        config.apply_env_overrides();
    ...
    async fn resolve_runtime_config_dirs_uses_env_config_dir_first() {
        ...
        crate::util::set_env_var("SAGENT_CONFIG_DIR", &explicit_config_dir);
        crate::util::remove_env_var("SAGENT_WORKSPACE");
        ...
        crate::util::remove_env_var("SAGENT_CONFIG_DIR");
        let _ = fs::remove_dir_all(default_config_dir).await;
    }
    ...
    async fn load_or_init_workspace_suffix_uses_legacy_config_layout() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("senagent_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let legacy_config_path = temp_home.join(".senagent").join("config.toml");
    ...
    async fn load_or_init_workspace_override_keeps_existing_legacy_config() {
        ...
        let legacy_config_dir = temp_home.join(".senagent");
...
"#,
    ...
    async fn load_or_init_decrypts_feishu_channel_secrets() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("senagent_test_home_{}", uuid::Uuid::new_v4()));
        let config_dir = temp_home.join(".senagent");
    ...
    async fn load_or_init_uses_persisted_active_workspace_marker() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("senagent_test_home_{}", uuid::Uuid::new_v4()));
        let temp_default_dir = temp_home.join(".senagent");
    ...
    async fn load_or_init_env_workspace_override_takes_priority_over_marker() {
        ...
        let original_home = std::env::var("HOME").ok();
        crate::util::set_env_var("HOME", &temp_home);
        crate::util::set_env_var("SAGENT_WORKSPACE", &env_workspace_dir);
    ...
    async fn persist_active_workspace_marker_is_cleared_for_default_config_dir() {
        let temp_home =
            std::env::temp_dir().join(format!("senagent_test_home_{}", uuid::Uuid::new_v4()));
        let default_config_dir = temp_home.join(".senagent");
...
"#,
    ...
    async fn env_override_empty_values_ignored() {
        ...
        crate::util::set_env_var("SAGENT_PROVIDER", "");
        config.apply_env_overrides();
    ...
    async fn env_override_gateway_port() {
        ...
        crate::util::set_env_var("SAGENT_GATEWAY_PORT", "8080");
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 8080);

        crate::util::remove_env_var("SAGENT_GATEWAY_PORT");
    }
    ...
    async fn env_override_gateway_host() {
        ...
        crate::util::set_env_var("SAGENT_GATEWAY_HOST", "0.0.0.0");
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        crate::util::remove_env_var("SAGENT_GATEWAY_HOST");
    }
    ...
    async fn env_override_require_pairing() {
        ...
        crate::util::set_env_var("SAGENT_REQUIRE_PAIRING", "false");
        config.apply_env_overrides();
        assert!(!config.gateway.require_pairing);

        crate::util::set_env_var("SAGENT_REQUIRE_PAIRING", "true");
        config.apply_env_overrides();
        assert!(config.gateway.require_pairing);

        crate::util::remove_env_var("SAGENT_REQUIRE_PAIRING");
    }
    ...
    async fn env_override_temperature() {
        ...
        crate::util::set_env_var("SAGENT_TEMPERATURE", "0.5");
        config.apply_env_overrides();
        assert!((config.default_temperature - 0.5).abs() < f64::EPSILON);

        crate::util::remove_env_var("SAGENT_TEMPERATURE");
    }
    ...
    async fn env_override_temperature_out_of_range_ignored() {
        ...
        crate::util::set_env_var("SAGENT_TEMPERATURE", "3.0");
        config.apply_env_overrides();
        assert!(
    ...
    async fn env_override_reasoning_enabled() {
        ...
        crate::util::set_env_var("SAGENT_REASONING_ENABLED", "false");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));

        crate::util::set_env_var("SAGENT_REASONING_ENABLED", "true");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(true));

        crate::util::remove_env_var("SAGENT_REASONING_ENABLED");
    }
    ...
    async fn env_override_reasoning_invalid_value_ignored() {
        ...
        crate::util::set_env_var("SAGENT_REASONING_ENABLED", "maybe");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));
    ...
    async fn env_override_reasoning_effort() {
        ...
        crate::util::set_env_var("SAGENT_REASONING_EFFORT", "HIGH");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("high"));

        crate::util::remove_env_var("SAGENT_REASONING_EFFORT");
    }
    ...
    async fn env_override_reasoning_effort_legacy_codex_env() {
        ...
        crate::util::set_env_var("SAGENT_CODEX_REASONING_EFFORT", "minimal");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("minimal"));

        crate::util::remove_env_var("SAGENT_CODEX_REASONING_EFFORT");
    }
    ...
    async fn env_override_storage_provider_config() {
        ...
        crate::util::set_env_var("SAGENT_STORAGE_PROVIDER", "qdrant");
        crate::util::set_env_var("SAGENT_STORAGE_DB_URL", "http://localhost:6333");
        crate::util::set_env_var("SAGENT_STORAGE_CONNECT_TIMEOUT_SECS", "15");
        ...
        crate::util::remove_env_var("SAGENT_STORAGE_PROVIDER");
        crate::util::remove_env_var("SAGENT_STORAGE_DB_URL");
        crate::util::remove_env_var("SAGENT_STORAGE_CONNECT_TIMEOUT_SECS");
    }
    ...
    async fn env_override_proxy_scope_services() {
        ...
        let mut config = Config::default();
        crate::util::set_env_var("SAGENT_PROXY_ENABLED", "true");
        crate::util::set_env_var("SAGENT_HTTP_PROXY", "http://127.0.0.1:7890");
        crate::util::set_env_var(
            "SAGENT_PROXY_SERVICES",
            "provider.openai, tool.http_request",
        );
        crate::util::set_env_var("SAGENT_PROXY_SCOPE", "services");
    ...
    async fn env_override_proxy_scope_environment_applies_process_env() {
        ...
        let mut config = Config::default();
        crate::util::set_env_var("SAGENT_PROXY_ENABLED", "true");
        crate::util::set_env_var("SAGENT_PROXY_SCOPE", "environment");
        crate::util::set_env_var("SAGENT_HTTP_PROXY", "http://127.0.0.1:7890");
        crate::util::set_env_var("SAGENT_HTTPS_PROXY", "http://127.0.0.1:7891");
        crate::util::set_env_var("SAGENT_NO_PROXY", "localhost,127.0.0.1");
...
[security.estop]
enabled = true
state_file = "~/.senagent/estop-state.json"
require_otp_to_resume = true
"#,
        );
...
workspace_dir = "/senagent-data/workspace"
config_path = "/senagent-data/.senagent/config.toml"
api_key = ""
default_provider = "openrouter"
