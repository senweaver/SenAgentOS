...
// in `~/.senagent/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.senagent/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(senagent_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: senagent_dir.join(".secret_key"),
            enabled,
        }
    }
