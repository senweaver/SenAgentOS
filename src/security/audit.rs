...
impl AuditLogger {
    ...
    ///
    /// If `config.sign_events` is true, requires `SAGENT_AUDIT_SIGNING_KEY` env var
    /// to be set with a hex-encoded 32-byte key. Fails if key is missing or invalid.
    pub fn new(config: AuditConfig, senagent_dir: PathBuf) -> Result<Self> {
        // Load and validate signing key if sign_events enabled
        let signing_key = if config.sign_events {
            let key_hex = std::env::var("SAGENT_AUDIT_SIGNING_KEY").map_err(|_| {
                anyhow::anyhow!("sign_events enabled but SAGENT_AUDIT_SIGNING_KEY not set")
            })?;

            let key_bytes = hex::decode(&key_hex)
                .map_err(|_| anyhow::anyhow!("SAGENT_AUDIT_SIGNING_KEY must be hex-encoded"))?;

            if key_bytes.len() != 32 {
                bail!(
                    "SAGENT_AUDIT_SIGNING_KEY must be 32 bytes (64 hex chars), got {}",
                    key_bytes.len()
                );
            }
        ...
        let log_path = senagent_dir.join(&config.log_path);
...
/// - If a record has a `signature` field and `SAGENT_AUDIT_SIGNING_KEY` is available,
...
pub fn verify_chain(log_path: &Path) -> Result<u64> {
    ...
    let signing_key = std::env::var("SAGENT_AUDIT_SIGNING_KEY")
        .ok()
...
mod tests {
    ...
    /// Mutex to serialize tests that read/write SAGENT_AUDIT_SIGNING_KEY env var.
    ...
    fn audit_event_with_actor() {
        ...
        assert_eq!(actor.username, Some("@senagent_user".to_string()));
    }
    ...
    fn signature_present_when_sign_events_enabled() -> Result<()> {
        ...
        let old_key = std::env::var("SAGENT_AUDIT_SIGNING_KEY").ok();
        defer! {
            if let Some(key) = old_key {
                crate::util::set_env_var("SAGENT_AUDIT_SIGNING_KEY", key);
            } else {
                crate::util::remove_env_var("SAGENT_AUDIT_SIGNING_KEY");
            }
        }

        let tmp = TempDir::new()?;
        let test_key = "a".repeat(64); // 64 hex chars = 32 bytes
        crate::util::set_env_var("SAGENT_AUDIT_SIGNING_KEY", &test_key);
    ...
    fn constructor_fails_if_sign_events_but_no_key() -> Result<()> {
        ...
        if let Err(e) = result {
            ...
            assert!(
                err_msg.contains("SAGENT_AUDIT_SIGNING_KEY not set"),
                "error: {}",
                err_msg
            );
        }
    ...
    fn constructor_fails_if_signing_key_invalid_hex() -> Result<()> {
        ...
        crate::util::set_env_var("SAGENT_AUDIT_SIGNING_KEY", "not-valid-hex");
    ...
    fn constructor_fails_if_signing_key_wrong_length() -> Result<()> {
        ...
        crate::util::set_env_var("SAGENT_AUDIT_SIGNING_KEY", &short_key);
        let tmp = TempDir::new()?;
        let config = AuditConfig {
            enabled: true,
            sign_events: true,
