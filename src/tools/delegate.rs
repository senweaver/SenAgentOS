...
mod tests {
    ...
    fn enriched_prompt_includes_tools_workspace_datetime() {
        ...
        let workspace = std::env::temp_dir().join(format!(
            "senagent_delegate_enrich_test_{}",
            uuid::Uuid::new_v4()
        ));
    ...
    fn enriched_prompt_loads_skills_from_scoped_directory() {
        let workspace = std::env::temp_dir().join(format!(
            "senagent_delegate_skills_test_{}",
            uuid::Uuid::new_v4()
        ));
    ...
    fn enriched_prompt_falls_back_to_default_skills_dir() {
        let workspace = std::env::temp_dir().join(format!(
            "senagent_delegate_fallback_test_{}",
            uuid::Uuid::new_v4()
        ));
    ...
    async fn cancel_task_nonexistent() {
        let workspace = std::env::temp_dir().join(format!(
            "senagent_delegate_cancel_miss_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn check_result_retrieves_persisted_background_result() {
        let workspace = std::env::temp_dir().join(format!(
            "senagent_delegate_check_retrieve_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn check_result_rejects_path_traversal() {
        let workspace = std::env::temp_dir().join(format!(
            "senagent_delegate_traversal_check_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn cancel_task_rejects_path_traversal() {
        let workspace = std::env::temp_dir().join(format!(
            "senagent_delegate_traversal_cancel_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
