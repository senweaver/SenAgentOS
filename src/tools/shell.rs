...
mod tests {
    ...
    async fn shell_does_not_leak_api_key() {
        ...
        let _g2 = EnvGuard::set("SENAGENTOS_API_KEY", "sk-test-secret-67890");
        ...
        assert!(
            ...
            "SENAGENTOS_API_KEY leaked to shell command output"
        );
    }
    ...
    async fn shell_allows_configured_env_passthrough() {
        let _guard = EnvGuard::set("SENAGENTOS_TEST_PASSTHROUGH", "db://unit-test");
        let tool = ShellTool::new(
            test_security_with_env_passthrough(&["SENAGENTOS_TEST_PASSTHROUGH"]),
            test_runtime(),
        );
        ...
        assert!(result
            .output
            .contains("SENAGENTOS_TEST_PASSTHROUGH=db://unit-test"));
    }
    ...
    async fn shell_requires_approval_for_medium_risk_command() {
        ...
        let denied = tool
            .execute(json!({"command": "touch senagent_shell_approval_test"}))
            .await
        ...
        let allowed = tool
            .execute(json!({
                "command": "touch senagent_shell_approval_test",
                "approved": true
            }))
            .await
        ...
        let _ =
            tokio::fs::remove_file(std::env::temp_dir().join("senagent_shell_approval_test")).await;
    }
