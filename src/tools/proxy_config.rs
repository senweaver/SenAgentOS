    ...
    async fn handle_set(&self, args: &Value) -> anyhow::Result<ToolResult> {
        ...
        if let Some(scope_raw) = args.get("scope") {
            ...
            proxy.scope = Self::parse_scope(scope).ok_or_else(|| {
                anyhow::anyhow!("Invalid scope '{scope}'. Use environment|senagent|services")
            })?;
        }
...
impl Tool for ProxyConfigTool {
    ...
    fn description(&self) -> &str {
        "Manage SenAgentOS proxy settings (scope: environment | senagent | services), including runtime and process env application"
    }
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "scope": {
                    "type": "string",
                    "description": "Proxy scope: environment | senagent | services"
                },
                "http_proxy": {
                    "type": ["string", "null"],
