...
//! Wraps a discovered MCP tool as a senagent [`Tool`] so it is dispatched
...
/// A senagent [`Tool`] backed by an MCP server tool.
///
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // SenAgentOS's security model injects `approved: bool` into built-in tool
...
mod tests {
    ...
    // SenAgentOS's security model injects `approved: bool` into built-in tool args.
