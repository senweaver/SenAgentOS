...
//!
//! Wire protocol (SenAgentOS serial JSON):
//! ```text
...
/// Default baud rate for SenAgentOS serial devices.
...
/// Serial transport for SenAgentOS hardware devices.
///
...
impl HardwareSerialTransport {
    ...
    /// Attempt a ping handshake to verify SenAgentOS firmware is running.
    ///
    /// Opens the port, sends `{"cmd":"ping","params":{}}`, waits up to
    /// `PING_TIMEOUT_MS` for a response with `data.firmware == "senagent"`.
    ///
    /// Returns `true` if a SenAgentOS device responds, `false` otherwise.
    ...
    pub async fn ping_handshake(&self) -> bool {
        ...
        match result {
            Ok(Ok(resp)) => {
                // Accept if firmware field is "senagent" (in data or top-level)
                resp.ok
                    && resp
                        .data
                        .get("firmware")
