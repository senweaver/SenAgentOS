...
pub struct HardwareBootResult {
    ...
    /// Content of `~/.senagent/hardware/` context files (HARDWARE.md, device
    /// profiles, and skills) for injection into the system prompt.
    pub context_files_prompt: String,
}

/// Load hardware context files from `~/.senagent/hardware/` and return them
/// concatenated as a single markdown string ready for system-prompt injection.
///
/// Reads (if they exist):
/// 1. `~/.senagent/hardware/HARDWARE.md`
/// 2. `~/.senagent/hardware/devices/<alias>.md` for each discovered alias
/// 3. All `~/.senagent/hardware/skills/*.md` files (sorted by name)
///
...
pub fn load_hardware_context_prompt(aliases: &[&str]) -> String {
    ...
    load_hardware_context_from_dir(&home.join(".senagent").join("hardware"), aliases)
}
...
///
/// Without the feature: loads plugin tools from `~/.senagent/tools/` only,
...
pub async fn boot(
    ...
    if uf2::find_rpi_rp2_mount().is_some() {
        ...
        tracing::info!("Say \"flash my pico\" to install SenAgentOS firmware automatically");
    }
...
/// Handle `senagent hardware` subcommands.
...
fn run_info(chip: &str) -> Result<()> {
    ...
    #[cfg(not(feature = "probe"))]
    {
        ...
        println!();
        println!("Then run: senagent hardware info --chip {}", chip);
        println!();
...
mod tests {
    ...
    fn hardware_context_contains_device_exec_rule() {
        ...
        if let Some(home) = directories::BaseDirs::new().map(|d| d.home_dir().to_path_buf()) {
            let hw_md = home.join(".senagent").join("hardware").join("HARDWARE.md");
            if hw_md.exists() {
