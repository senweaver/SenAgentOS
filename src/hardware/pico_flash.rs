...
//! `pico_flash` tool — flash SenAgentOS firmware to a Pico in BOOTSEL mode.
//!
//! # Happy path
...
//! 8. Tool returns success; user restarts SenAgentOS to get `pico0`.
...
/// Tool: flash SenAgentOS MicroPython firmware to a Pico in BOOTSEL mode.
///
...
/// calls work immediately without restarting SenAgentOS.
...
impl Tool for PicoFlashTool {
    ...
    fn description(&self) -> &str {
        "Flash SenAgentOS firmware to a Raspberry Pi Pico in BOOTSEL mode. \
                ...
                return Ok(ToolResult {
                    ...
                    error: Some(format!(
                        "UF2 copied to {} but serial port did not appear within {PORT_WAIT_SECS}s. \
                         ...
                         mpremote connect <port> cp ~/.senagent/firmware/pico/main.py :main.py + reset",
                        mount.display()
                    )),
                });
            }
        ...
        match final_port {
            Some(p) => {
                ...
                let suffix = if reconnected {
                    ...
                    "Restart SenAgentOS to reconnect as pico0."
                };
                Ok(ToolResult {
                    success: true,
                    output: format!(
            ...
            None => Ok(ToolResult {
                ...
                output: format!(
                    "Pico flashed and main.py deployed. \
                         ...
                         unplug and replug the Pico, then restart SenAgentOS to connect as pico0."
                ),
                error: None,
            }),
        }
