...
//!    `~/.senagent/firmware/pico/` if they aren't there yet.
...
const PICO_UF2: &[u8] = include_bytes!("../../firmware/pico/senagent-pico.uf2");

/// SenAgentOS serial protocol handler — written to the Pico after MicroPython boots.
...
/// Ensure `~/.senagent/firmware/pico/` exists and contains the bundled assets.
///
...
pub fn ensure_firmware_dir() -> Result<PathBuf> {
    ...
    let uf2_path = firmware_dir.join("senagent-pico.uf2");
    if !uf2_path.exists() {
        if PICO_UF2.len() < 8 || PICO_UF2[..4] != UF2_MAGIC1 {
            bail!(
                "Bundled UF2 is a placeholder — download the real MicroPython UF2 from \
                 ...
                 src/firmware/pico/senagent-pico.uf2, then rebuild SenAgentOS."
            );
        }
...
pub async fn flash_uf2(mount_point: &Path, firmware_dir: &Path) -> Result<()> {
    let uf2_src = firmware_dir.join("senagent-pico.uf2");
    ...
    if data.len() < 8 || data[..4] != UF2_MAGIC1 {
        bail!(
            "UF2 at {} does not look like a valid UF2 file (magic mismatch). \
             ...
             the existing file so SenAgentOS can re-extract it.",
            uf2_src.display()
        );
    }
    ...
    bail!(
        "All copy methods failed. Run this command manually, then restart SenAgentOS:\n\
...
mod tests {
    ...
    #[test]
    fn pico_main_py_contains_senagent_marker() {
        let src = std::str::from_utf8(PICO_MAIN_PY).expect("main.py is not valid UTF-8");
        assert!(
            src.contains("senagent"),
            "main.py should contain 'senagent' firmware marker"
        );
    }
