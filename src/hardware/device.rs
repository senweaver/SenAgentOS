...
pub enum DeviceKind {
    ...
    Nucleo,
    /// Unknown VID that passed the SenAgentOS firmware ping handshake.
    Generic,
...
impl DeviceRegistry {
    ...
    ///    if the device responds with SenAgentOS firmware.
        ...
        for info in scan_serial_devices() {
            ...
            let probe_transport = if !is_known_vid {
                ...
                if !probe.ping_handshake().await {
                    tracing::debug!(
                        port = %info.port_path,
                        "skipping unknown device: no SenAgentOS firmware response"
                    );
                    continue;
                }
                Some(probe)
...
impl DeviceRegistry {
    ...
    /// SenAgentOS firmware is alive, and re-attaches the transport.
    ///
