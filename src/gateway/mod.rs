...
/// Read gateway request timeout from `SENAGENTOS_GATEWAY_TIMEOUT_SECS` env var
...
pub fn gateway_request_timeout_secs() -> u64 {
    std::env::var("SENAGENTOS_GATEWAY_TIMEOUT_SECS")
        .ok()
...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    ...
    let whatsapp_app_secret: Option<Arc<str>> = std::env::var("SENAGENTOS_WHATSAPP_APP_SECRET")
        .ok()
        .and_then(|secret| {
    ...
    // Priority: environment variable > config file
    let linq_signing_secret: Option<Arc<str>> = std::env::var("SENAGENTOS_LINQ_SIGNING_SECRET")
        .ok()
        .and_then(|secret| {
            let secret = secret.trim();
            (!secret.is_empty()).then(|| secret.to_owned())
    ...
    let nextcloud_talk_webhook_secret: Option<Arc<str>> =
        std::env::var("SENAGENTOS_NEXTCLOUD_TALK_WEBHOOK_SECRET")
            .ok()
            .and_then(|secret| {
                let secret = secret.trim();
                (!secret.is_empty()).then(|| secret.to_owned())
    ...
    println!("???? SenAgentOS Gateway listening on http://{display_addr}{pfx}");
    ...
    if let Some(code) = pairing.pairing_code() {
        ...
        println!("     To pair a new device: senagent gateway get-paircode --new");
        println!();
    } else {
    ...
    let a2a_state = a2a::build_a2a_state("senagent", &format!("http://{}:{}", host, actual_port));
    ...
    let inner = Router::new()
        ...
        .route("/api/channels", get(api::handle_api_channels_get))
        .route("/api/channels", put(api::handle_api_channels_put))
        .route("/api/tools", get(api::handle_api_tools))
        .route("/api/skills", get(api::handle_api_skills_get).put(api::handle_api_skills_put))
        ...
        loop {
            tokio::select! {
                ...
                _ = shutdown_signal.changed() => {
                    tracing::info!("???? SenAgentOS Gateway shutting down...");
                    break;
                }
            }
        }
...
mod tests {
    ...
    fn gateway_timeout_falls_back_to_default() {
        ...
        crate::util::remove_env_var("SENAGENTOS_GATEWAY_TIMEOUT_SECS");
    ...
    async fn metrics_endpoint_renders_prometheus_output() {
        ...
        let body = response.into_body().collect().await.unwrap().to_bytes();
        let text = String::from_utf8(body.to_vec()).unwrap();
        assert!(text.contains("senagent_heartbeat_ticks_total 1"));
    }
