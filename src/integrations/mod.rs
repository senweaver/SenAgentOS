...
fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `senagent onboard` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: senagent onboard --channels-only");
            println!("    4. Start: senagent channel start");
        }
        "Discord" => {
            println!("  Setup:");
            ...
            println!("    4. Run: senagent onboard --channels-only");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "OpenRouter" => {
            ...
            println!("    2. Run: senagent onboard");
        ...
        "Browser" => {
            println!("  Built-in:");
            println!("    SenAgentOS can control Chrome/Chromium for web tasks.");
            println!("    Uses headless browser automation.");
        }
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.senagent/workspace/cron/");
            println!("    Run: senagent cron list");
        }
        "Weather" => {
            println!("  Built-in:");
        ...
        "Webhooks" => {
            ...
            println!("    Run: senagent gateway");
        }
        _ => {
            if status == IntegrationStatus::ComingSoon {
                println!("  This integration is planned. Stay tuned!");
                println!("  Track progress: https://github.com/senweaver/SenAgentOS");
            }
        }
    }
