...
fn load_env() {
    ...
    let candidates = [
        std::env::var_os("SENAGENT_WORKSPACE")
            .map(PathBuf::from)
            .filter(|p| p.is_absolute())
            .map(|p| p.join(".env")),
        std::env::current_dir().ok().map(|p| p.join(".env")),
        std::env::current_exe().ok().map(|p| p.parent().unwrap().join(".env")),
    ]
    .into_iter()
    .flatten()
    .filter(|p| p.is_file());
...
fn print_no_command_help() -> Result<()> {
    ...
    println!("Try `senagent onboard` to initialize your workspace.");
    println!();
...
/// `SenAgentOS` - Zero overhead. Zero compromise. 100% Rust.
#[derive(Parser, Debug)]
#[command(name = "senagent")]
...
Examples:
  senagent agent                              # interactive session
  senagent agent -m \"Summarize today's logs\"  # single message
  senagent agent -p anthropic --model claude-sonnet-4-20250514
  senagent agent --peripheral nucleo-f401re:/dev/ttyACM0")]
    Agent {
...
Examples:
  senagent gateway start              # start gateway
  senagent gateway restart            # restart gateway
  senagent gateway get-paircode       # show pairing code")]
    Gateway {
        #[command(subcommand)]
        gateway_command: Option<senagentos::GatewayCommands>,
    },
...
Examples:
  senagent acp                        # start ACP server
  senagent acp --max-sessions 5       # limit concurrent sessions")]
    Acp {
...
Launches the full SenAgentOS runtime: gateway server, all configured \
channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and \
the cron scheduler. This is the recommended way to run SenAgentOS in \
production or as an always-on assistant.

Use 'senagent service install' to register the daemon as an OS \
service (systemd/launchd) for auto-start on boot.

Examples:
  senagent daemon                   # use config defaults
  senagent daemon -p 9090           # gateway on port 9090
  senagent daemon --host 127.0.0.1  # localhost only")]
    Daemon {
    ...
    /// Examples:
    /// - `senagent estop`
    /// - `senagent estop --level network-kill`
    /// - `senagent estop --level domain-block --domain "*.chase.com"`
    /// - `senagent estop --level tool-freeze --tool shell --tool browser`
    /// - `senagent estop status`
    /// - `senagent estop resume --network`
    /// - `senagent estop resume --domain "*.chase.com"`
    /// - `senagent estop resume --tool shell`
    Estop {
        #[command(subcommand)]
        estop_command: Option<EstopSubcommands>,

        /// Level used when engaging estop from `senagent estop`.
        #[arg(long, value_enum)]
...
Examples:
  senagent cron list
  senagent cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York --agent
  senagent cron add '*/30 * * * *' 'Check system health' --agent
  senagent cron add '*/5 * * * *' 'echo ok'
  senagent cron add-at 2025-01-15T14:00:00Z 'Send reminder' --agent
  senagent cron add-every 60000 'Ping heartbeat'
  senagent cron once 30m 'Run backup in 30 minutes' --agent
  senagent cron pause <task-id>
  senagent cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    Cron {
        #[command(subcommand)]
...
Add, remove, list, send, and health-check channels that connect SenAgentOS \
to messaging platforms. Supported channel types: telegram, discord, \
slack, whatsapp, matrix, imessage, email.

Examples:
  senagent channel list
  senagent channel doctor
  senagent channel add telegram '{\"bot_token\":\"...\",\"name\":\"my-bot\"}'
  senagent channel remove my-bot
  senagent channel bind-telegram senagent_user
  senagent channel send 'Alert!' --channel-id telegram --recipient 123456789")]
    Channel {
        #[command(subcommand)]
...
Examples:
  senagent hardware discover
  senagent hardware introspect /dev/ttyACM0
  senagent hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
        hardware_command: senagentos::HardwareCommands,
    },
...
Examples:
  senagent peripheral list
  senagent peripheral add nucleo-f401re /dev/ttyACM0
  senagent peripheral add rpi-gpio native
  senagent peripheral flash --port /dev/cu.usbmodem12345
  senagent peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
        peripheral_command: senagentos::PeripheralCommands,
    },
...
Examples:
  senagent memory stats
  senagent memory list
  senagent memory list --category core --limit 10
  senagent memory get <key>
  senagent memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Manage SenAgentOS configuration.
...
Examples:
  senagent config schema              # print JSON Schema to stdout
  senagent config schema > schema.json")]
    Config {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Check for and apply SenAgentOS updates.
...
Examples:
  senagent update                      # download and install latest
  senagent update --check              # check only, don't install
  senagent update --force              # install without confirmation
  senagent update --version 0.6.0      # install specific version")]
    Update {
    ...
    #[command(long_about = "\
Run diagnostic self-tests to verify the SenAgentOS installation.
...
Examples:
  senagent self-test             # full suite
  senagent self-test --quick     # quick checks only (no network)")]
    SelfTest {
    ...
    #[command(long_about = "\
Generate shell completion scripts for `senagent`.

The script is printed to stdout so it can be sourced directly:

Examples:
  source <(senagent completions bash)
  senagent completions zsh > ~/.zfunc/_senagent
  senagent completions fish > ~/.config/fish/completions/senagent.fish")]
    Completions {
        /// Target shell
        #[arg(value_enum)]
    ...
    #[command(long_about = "\
Launch the SenAgentOS companion desktop app.
...
Examples:
  senagent desktop              # launch the companion app
  senagent desktop --install    # download and install it")]
    Desktop {
...
async fn main() -> Result<()> {
    ...
    if let Some(config_dir) = &cli.config_dir {
        ...
        crate::util::set_env_var("SENAGENTOS_CONFIG_DIR", &config_dir);
    }
    ...
    // or set SENAGENTOS_INTERACTIVE=1 to force interactive mode when TTY
    // detection fails.  This means `curl … | bash` and
    // `senagent onboard --api-key …` both take the fast path, while a bare
    // `senagent onboard` in a terminal launches the wizard.
    ...
    } = &cli.command
    {
        ...
        if reinit {
            ...
            if senagent_dir.exists() {
                let timestamp = chrono::Local::now().format("%Y%m%d%H%M%S");
                let backup_dir = format!("{}.backup.{}", senagent_dir.display(), timestamp);

                println!("⚠️  Reinitializing SenAgentOS configuration...");
                println!("   Current config directory: {}", senagent_dir.display());
                println!(
                ...
                tokio::fs::rename(&senagent_dir, &backup_dir)
                    .await
                    .with_context(|| {
        ...
        let env_interactive = std::env::var("SENAGENTOS_INTERACTIVE").as_deref() == Ok("1");
        ...
        if std::env::var("SENAGENTOS_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!("Initialized OTP secret for SenAgentOS.");
    ...
    match cli.command {
        ...
        Commands::Gateway { gateway_command } => {
            match gateway_command {
                Some(senagentos::GatewayCommands::Restart { port, host }) => {
                    let (port, host) = resolve_gateway_addr(&config, port, host);
                    let addr = format!("{host}:{port}");
                    info!("🔄 Restarting SenAgentOS Gateway on {addr}");
                ...
                Some(senagentos::GatewayCommands::GetPaircode { new }) => {
                    ...
                    match fetch_paircode(host, port, new).await {
                        ...
                        Err(e) => {
                            ...
                            println!("     senagent gateway start");
                        }
                    }
                    Ok(())
                }
                Some(senagentos::GatewayCommands::Start { port, host }) => {
                    let (port, host) = resolve_gateway_addr(&config, port, host);
                    log_gateway_start(&host, port);
                    Box::pin(gateway::run_gateway(&host, port, config)).await
                }
        ...
        Commands::Daemon { port, host } => {
            ...
            if port == 0 {
                info!("🧠 Starting SenAgentOS Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting SenAgentOS Daemon on {host}:{port}");
            }
        ...
        Commands::Desktop {
            ...
            let download_url = "https://www.senagent-os.ai/download";

            if do_install {
                println!("Download the SenAgentOS companion app:");
                println!();
                #[cfg(target_os = "macos")]
                {
                    ...
                    println!("  brew install --cask senagent");
                }
                #[cfg(target_os = "linux")]
                {
            ...
            let desktop_bin = {
                ...
                // 1. macOS: check /Applications/SenAgentOS.app
                #[cfg(target_os = "macos")]
                {
                    let app_paths = [
                        PathBuf::from("/Applications/SenAgentOS.app/Contents/MacOS/SenAgentOS"),
                        PathBuf::from(std::env::var("HOME").unwrap_or_default())
                            .join("Applications/SenAgentOS.app/Contents/MacOS/SenAgentOS"),
                    ];
                ...
                if found.is_none() {
                    if let Ok(exe) = std::env::current_exe() {
                        let sibling = exe.with_file_name("senagent-desktop");
                        if sibling.is_file() {
                            found = Some(sibling);
                        }
                    }
                ...
                // 3. ~/.cargo/bin/senagent-desktop or ~/.local/bin/senagent-desktop
                if found.is_none() {
                    if let Some(home) = std::env::var_os("HOME") {
                        let home = PathBuf::from(home);
                        for dir in &[".cargo/bin", ".local/bin"] {
                            let candidate = home.join(dir).join("senagent-desktop");
                ...
                if found.is_none() {
                    if let Ok(path) = which::which("senagent-desktop") {
                        found = Some(path);
                    }
                }
            ...
            match desktop_bin {
                Some(bin) => {
                    println!("Launching SenAgentOS companion app...");
                ...
                None => {
                    println!("SenAgentOS companion app is not installed.");
                    println!();
                    println!("  Download it at: {download_url}");
                    println!("  Or run: senagent desktop --install");
                    println!();
        ...
        Commands::Plugin { plugin_command } => match plugin_command {
            PluginCommands::List => {
                let host = senagentos::plugins::host::PluginHost::new(&config.workspace_dir)?;
            ...
            PluginCommands::Install { source } => {
                let mut host = senagentos::plugins::host::PluginHost::new(&config.workspace_dir)?;
                host.install(&source)?;
...
fn log_gateway_start(host: &str, port: u16) {
    if port == 0 {
        info!("🚀 Starting SenAgentOS Gateway on {host} (random port)");
    } else {
        info!("🚀 Starting SenAgentOS Gateway on {host}:{port}");
    }
}
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `senagent auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `senagent auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
            ...
            match provider.as_str() {
                "openai-codex" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `senagent auth login --provider openai-codex` first."
                        )
                    })?;
                ...
                "gemini" => {
                    let pending = load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending Gemini login found. Run `senagent auth login --provider gemini` first."
                        )
                    })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `senagent auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `senagent auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["senagent", "completions", shell])
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["senagent", "onboard", "--force"])
    ...
    fn onboard_cli_rejects_removed_interactive_flag() {
        ...
        assert!(Cli::try_parse_from(["senagent", "onboard", "--interactive"]).is_err());
    }

    #[test]
    fn onboard_cli_parses_quick_flag() {
        let cli = Cli::try_parse_from(["senagent", "onboard", "--quick"])
    ...
    fn onboard_cli_quick_and_channels_only_conflict() {
        ...
        let cli = Cli::try_parse_from(["senagent", "onboard", "--quick", "--channels-only"]);
        assert!(
            cli.is_ok(),
    ...
    fn onboard_cli_bare_parses() {
        let cli = Cli::try_parse_from(["senagent", "onboard"]).expect("bare onboard should parse");
    ...
    fn cli_parses_estop_default_engage() {
        let cli = Cli::try_parse_from(["senagent", "estop"]).expect("estop command should parse");
    ...
    fn cli_parses_estop_resume_domain() {
        let cli = Cli::try_parse_from(["senagent", "estop", "resume", "--domain", "*.chase.com"])
    ...
    fn agent_command_parses_with_temperature() {
        let cli = Cli::try_parse_from(["senagent", "agent", "--temperature", "0.5"])
    ...
    fn agent_command_parses_without_temperature() {
        let cli = Cli::try_parse_from(["senagent", "agent", "--message", "hello"])
    ...
    fn agent_command_parses_session_state_file() {
        let cli =
            Cli::try_parse_from(["senagent", "agent", "--session-state-file", "session.json"])
