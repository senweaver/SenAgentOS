...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to SenAgentOS — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 9, "Channels (How You Talk to SenAgentOS)");
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            set_env_var("SENAGENTOS_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub async fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to SenAgentOS)");
...
/// Use `senagent onboard` or `senagent onboard --api-key sk-... --provider openrouter --memory sqlite|lucid`.
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    if let Ok(custom_config_dir) = std::env::var("SENAGENTOS_CONFIG_DIR") {
    ...
    if let Ok(custom_workspace) = std::env::var("SENAGENTOS_WORKSPACE") {
    ...
    let config_dir = home.join(".senagent");
    (config_dir.clone(), config_dir.join("workspace"))
}

fn homebrew_prefix_for_exe(exe: &Path) -> Option<&'static str> {
    let exe = exe.to_string_lossy();
    if exe == "/opt/homebrew/bin/senagent"
        || exe.starts_with("/opt/homebrew/Cellar/senagent/")
        || exe.starts_with("/opt/homebrew/opt/senagent/")
    {
        return Some("/opt/homebrew");
    }

    if exe == "/usr/local/bin/senagent"
        || exe.starts_with("/usr/local/Cellar/senagent/")
        || exe.starts_with("/usr/local/opt/senagent/")
    {
...
fn quick_setup_homebrew_service_note(
    ...
    let service_root = Path::new(prefix).join("var").join("senagent");
    ...
    Some(format!(
        "Homebrew service note: `brew services` uses {} (config {}) by default. Your onboarding just wrote {}. If you plan to run SenAgentOS as a service, copy or link this workspace first.",
...
async fn run_quick_setup_with_home(
    ...
    let (senagent_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    ...
    if credential_override.is_none() {
        if provider_supports_keyless_local_usage(&provider_name) {
            println!("    1. Chat:     senagent agent -m \"Hello!\"");
            println!("    2. Gateway:  senagent gateway");
            println!("    3. Status:   senagent status");
            ...
            if canonical_provider_name(&provider_name) == "copilot" {
                ...
                println!(
                    "    1. Login:             senagent auth login --provider {}",
                    provider_name
                );
                println!("    2. Chat:              senagent agent -m \"Hello!\"");
                println!("    3. Gateway:           senagent gateway");
                println!("    4. Status:            senagent status");
            }
        } else {
            let env_var = provider_env_var(&provider_name);
            println!("    1. Set your API key:  export {env_var}=\"sk-...\"");
            println!("    2. Or edit:           ~/.senagent/config.toml");
            println!("    3. Chat:              senagent agent -m \"Hello!\"");
            println!("    4. Gateway:           senagent gateway");
        }
    } else {
        println!("    1. Chat:     senagent agent -m \"Hello!\"");
        println!("    2. Gateway:  senagent gateway");
...
pub async fn run_models_refresh(
    ...
    if !force {
        ...
        .await?
        {
            ...
            println!();
            println!(
                "Tip: run `senagent models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
pub async fn run_models_list(config: &Config, provider_override: Option<&str>) -> Result<()> {
    ...
    let Some(cached) = cached else {
        println!();
        println!(
            "  No cached models for '{provider_name}'. Run: senagent models refresh --provider {provider_name}"
        );
        println!();
        return Ok(());
    };
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("SenAgentOS will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how SenAgentOS connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("SenAgentOS uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("SenAgentOS can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("SenAgentOS can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::new()
        .with_prompt("  How should SenAgentOS interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how SenAgentOS stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to SenAgentOS from anywhere.");
        ...
        match choice {
            ChannelMenuChoice::Telegram => {
                ...
                println!(
                    ...
                    style("— talk to SenAgentOS from Telegram").dim()
                );
            ...
            ChannelMenuChoice::Discord => {
                ...
                println!(
                    ...
                    style("— talk to SenAgentOS from Discord").dim()
                );
            ...
            ChannelMenuChoice::Slack => {
                ...
                println!(
                    ...
                    style("— talk to SenAgentOS from Slack").dim()
                );
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::new()
                        ...
                        .default("~/.senagent/state/whatsapp-web/session.db")
                        .interact_text()?;
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("senagent-whatsapp-verify")
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via SENAGENTOS_WHATSAPP_APP_SECRET env var
                    session_path: None,
                    pair_phone: None,
                    pair_code: None,
                    allowed_numbers,
            ...
            ChannelMenuChoice::Lark | ChannelMenuChoice::Feishu => {
                ...
                println!(
                    ...
                    style(format!("— talk to SenAgentOS from {provider_label}")).dim()
                );
                print_bullet(&format!(
            ...
            ChannelMenuChoice::Nostr => {
                ...
                print_bullet("SenAgentOS will listen for encrypted DMs on Nostr relays.");
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("SenAgentOS is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if config.api_key.is_none() && !provider_supports_keyless_local_usage(provider) {
        if provider == "openai-codex" {
            ...
            println!(
                "       {}",
                style("senagent auth login --provider openai-codex --device-code").yellow()
            );
            ...
            println!(
                "       {}",
                style(
                    "or: senagent auth paste-token --provider anthropic --auth-kind authorization"
                )
                .yellow()
            );
        } else {
    ...
    if has_channels {
        ...
        println!("       {}", style("senagent channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("senagent agent -m \"Hello, SenAgentOS!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("senagent agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("senagent status").yellow());
...
mod tests {
    ...
    async fn quick_setup_model_override_persists_to_config_toml() {
        ...
        let _workspace_env = EnvVarGuard::unset("SENAGENTOS_WORKSPACE");
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        ...
        let _config_env = EnvVarGuard::unset("SENAGENTOS_CONFIG_DIR");
        let tmp = TempDir::new().unwrap();
        let senagent_dir = tmp.path().join(".senagent");
        let config_path = senagent_dir.join("config.toml");

        tokio::fs::create_dir_all(&senagent_dir).await.unwrap();
    ...
    async fn quick_setup_respects_zero_claw_workspace_env_layout() {
        let _env_guard = env_lock().lock().await;
        let tmp = TempDir::new().unwrap();
        let workspace_root = tmp.path().join("senagent-data");
        ...
        .await
        .expect("quick setup should honor SENAGENTOS_WORKSPACE");
    ...
    fn homebrew_prefix_for_exe_detects_supported_layouts() {
        assert_eq!(
            homebrew_prefix_for_exe(Path::new("/opt/homebrew/bin/senagent")),
            Some("/opt/homebrew")
        );
        assert_eq!(
            homebrew_prefix_for_exe(Path::new(
                "/opt/homebrew/Cellar/senagent/0.5.0/bin/senagent",
            )),
            Some("/opt/homebrew")
        );
        assert_eq!(
            homebrew_prefix_for_exe(Path::new("/usr/local/bin/senagent")),
            Some("/usr/local")
        );
        assert_eq!(homebrew_prefix_for_exe(Path::new("/tmp/senagent")), None);
    }

    #[test]
    fn quick_setup_homebrew_service_note_mentions_service_workspace() {
        let note = quick_setup_homebrew_service_note(
            Path::new("/Users/alix/.senagent/config.toml"),
            Path::new("/Users/alix/.senagent/workspace"),
            Path::new("/opt/homebrew/bin/senagent"),
        )
        .expect("homebrew installs should emit a service workspace note");

        assert!(note.contains("/opt/homebrew/var/senagent/workspace"));
        assert!(note.contains("/opt/homebrew/var/senagent/config.toml"));
        assert!(note.contains("/Users/alix/.senagent/config.toml"));
    }
    ...
    async fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            ...
            .unwrap();
        assert!(
            identity.contains("**Name:** SenAgentOS"),
            "should default agent name to SenAgentOS"
        );
    ...
    async fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = tokio::fs::read_to_string(tmp.path().join("SOUL.md"))
            .await
            .unwrap();
        assert!(soul.contains("SenAgentOS-v2"));
    }
