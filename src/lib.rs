...
Examples:
  senagent gateway start              # use config defaults
  senagent gateway start -p 8080      # listen on port 8080
  senagent gateway start --host 0.0.0.0   # requires [gateway].allow_public_bind=true or a tunnel
  senagent gateway start -p 0         # random available port")]
    Start {
...
Examples:
  senagent gateway restart            # restart with config defaults
  senagent gateway restart -p 8080    # restart on port 8080")]
    Restart {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
...
Examples:
  senagent gateway get-paircode       # show current pairing code
  senagent gateway get-paircode --new # generate a new pairing code")]
    GetPaircode {
...
Examples:
  ...
  senagent channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  senagent channel bind-telegram 123456789")]
    BindTelegram {
...
Examples:
  senagent channel send 'Someone is near your device.' --channel-id telegram --recipient 123456789
  senagent channel send 'Build succeeded!' --channel-id discord --recipient 987654321")]
    Send {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `SenAgentOS` workspace
    Openclaw {
...
Examples:
  senagent cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  senagent cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  senagent cron add-every 60000 'Ping heartbeat'     # every minute
  senagent cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  senagent cron once 30m 'Run backup in 30 minutes'
  senagent cron once 2h 'Follow up on deployment'
  senagent cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Treat the argument as an agent prompt instead of a shell command
...
Examples:
  senagent cron update <task-id> --expression '0 8 * * *'
  senagent cron update <task-id> --tz Europe/London --name 'Morning check'
  senagent cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  senagent hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  senagent peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash SenAgentOS firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash SenAgentOS firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  senagent peripheral flash
  senagent peripheral flash --port /dev/cu.usbmodem12345
  senagent peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash SenAgentOS firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
