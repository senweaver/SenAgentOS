...
pub struct ProviderRuntimeOptions {
    ...
    pub senagent_dir: Option<PathBuf>,
    ...
    /// These are merged from the config file and `SENAGENTOS_EXTRA_HEADERS` env var.
...
pub fn provider_runtime_options_from_config(
    ...
    ProviderRuntimeOptions {
        ...
        senagent_dir: config.config_path.parent().map(PathBuf::from),
...
/// 3. Generic fallback variables (`SENAGENTOS_API_KEY`, `API_KEY`)
///
...
fn resolve_provider_credential(name: &str, credential_override: Option<&str>) -> Option<String> {
    ...
    for env_var in ["SENAGENTOS_API_KEY", "API_KEY"] {
        if let Ok(value) = std::env::var(env_var) {
            let value = value.trim();
            if !value.is_empty() {
                return Some(value.to_string());
...
fn create_provider_with_url_and_options(
    ...
    match name {
        ...
        "ollama" => {
            let env_url = std::env::var("SENAGENTOS_PROVIDER_URL").ok();
        ...
        "gemini" | "google" | "google-gemini" => {
            let state_dir = options.senagent_dir.clone().unwrap_or_else(|| {
                directories::UserDirs::new().map_or_else(
                    || PathBuf::from(".senagent"),
                    |dirs| dirs.home_dir().join(".senagent"),
                )
            });
        ...
        _ => anyhow::bail!(
            "Unknown provider: {name}. Check README for supported providers or run `senagent onboard` to reconfigure.\n\
...
/// Return the list of all known providers for display in `senagent providers list`.
///
...
mod tests {
    ...
    fn resolve_qwen_oauth_context_prefers_explicit_override() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/senagent-qwen-oauth-home-{}", std::process::id());
    ...
    fn resolve_qwen_oauth_context_uses_env_token_and_resource_url() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/senagent-qwen-oauth-home-{}-env", std::process::id());
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, Some("oauth-token"));
    ...
    fn resolve_qwen_oauth_context_reads_cached_credentials_file() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/senagent-qwen-oauth-home-{}-file", std::process::id());
    ...
    fn resolve_qwen_oauth_context_placeholder_does_not_use_dashscope_fallback() {
        ...
        let fake_home = format!(
            "/tmp/senagent-qwen-oauth-home-{}-placeholder",
            std::process::id()
        );
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, None);
    ...
    fn resolve_provider_credential_opencode_go_env() {
        ...
        let _senagent_guard = EnvGuard::set("SENAGENTOS_API_KEY", None);
    ...
    fn provider_runtime_options_extra_headers_passed_through() {
        ...
        extra_headers.insert("X-Title".to_string(), "senagent".to_string());
        let options = ProviderRuntimeOptions {
            extra_headers,
            ..ProviderRuntimeOptions::default()
        };
        assert_eq!(options.extra_headers.len(), 1);
        assert_eq!(options.extra_headers.get("X-Title").unwrap(), "senagent");
    }

    #[test]
    fn env_provider_url_overrides_api_url() {
        crate::util::set_env_var("SENAGENTOS_PROVIDER_URL", "http://env-ollama:11434");
        ...
        crate::util::remove_env_var("SENAGENTOS_PROVIDER_URL");
    }
}
