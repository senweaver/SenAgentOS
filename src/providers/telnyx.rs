...
///
/// ```rust,ignore
/// use senagentos::providers::telnyx::TelnyxProvider;
/// use senagentos::providers::Provider;
///
...
impl TelnyxProvider {
    ...
    /// 2. `SENAGENTOS_API_KEY` environment variable (fallback)
...
impl Provider for TelnyxProvider {
    async fn chat_with_system(
        ...
        let api_key = self.api_key.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "Telnyx API key not set. Set TELNYX_API_KEY environment variable or run `senagent onboard`."
            )
        })?;
