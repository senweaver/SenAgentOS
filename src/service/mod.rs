...
const SERVICE_LABEL: &str = "com.senagent.daemon";
const WINDOWS_TASK_NAME: &str = "SenAgentOS Daemon";
...
/// Returns whether the SenAgentOS daemon service is currently running.
...
fn is_running_linux() -> bool {
    ...
    if run_capture(Command::new("systemctl").args(["--user", "is-active", "senagent.service"]))
        .map(|out| out.trim() == "active")
        .unwrap_or(false)
    {
        return true;
    }
    run_capture(Command::new("rc-service").args(["senagent", "status"]))
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "senagent.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["senagent", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "senagent.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["senagent", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "senagent.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["senagent", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["senagent", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/senagent");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn logs_linux(config: &Config, init_system: InitSystem, lines: usize, follow: bool) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            // OpenRC logs go to /var/log/senagent/error.log (as configured in the init script)
            let log_file = Path::new("/var/log/senagent/error.log");
            if !log_file.exists() {
                // Fall back to access log
                let access_log = Path::new("/var/log/senagent/access.log");
                if !access_log.exists() {
                    bail!("No log files found at /var/log/senagent/. Is the service installed?");
                }
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/senagent");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "senagent", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove senagent from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/senagent)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
/// corresponding `var/senagent` directory.
///
...
fn detect_homebrew_var_dir(exe: &Path) -> Option<PathBuf> {
    ...
    // Symlinked binary: <prefix>/bin/senagent
    // Cellar binary:    <prefix>/Cellar/senagent/<version>/bin/senagent
    let prefix = if path_str.contains("/Cellar/") {
        // Walk up from .../Cellar/senagent/<ver>/bin/senagent to the prefix
        ...
        while let Some(parent) = ancestor.parent() {
            ...
            if ancestor.file_name().map_or(false, |n| n == "Cellar") {
                ...
                return ancestor.parent().map(|p| p.join("var").join("senagent"));
            }
        }
        return None;
    } else if let Some(bin_parent) = exe.parent() {
        // <prefix>/bin/senagent → check if <prefix>/Cellar exists (Homebrew marker)
    ...
    prefix.map(|p| p.join("var").join("senagent"))
}
...
fn install_macos(config: &Config) -> Result<()> {
    ...
    // data so that `brew services start senagent` works out of the box.
    ...
    // When running under Homebrew, inject SENAGENTOS_CONFIG_DIR and
...
"#,
    ...
    println!("   Start with: senagent service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "senagent.service"]));
...
/// Check if the senagent user exists and has expected properties.
...
fn check_senagent_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "senagent"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser senagent && delgroup senagent",
            "addgroup -S senagent && adduser -S -s /sbin/nologin -H -D -G senagent senagent",
        )
    } else {
        ("userdel senagent", "useradd -r -s /sbin/nologin senagent")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'senagent' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/senagent" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: senagent user has home directory '{}' (expected /var/lib/senagent or /nonexistent)",
                        home
                    );
                }
...
fn ensure_senagent_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "senagent"]).output();
    if let Ok(output) = output {
        if output.status.success() {
            return check_senagent_user();
        }
    }
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "senagent"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "senagent"])
                .output()
                .context("Failed to create senagent group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create senagent group: {}", stderr.trim());
            }
            println!("✅ Created system group: senagent");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create senagent user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create senagent user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "senagent"])
            .output()
            .context("Failed to create senagent user")?;
    ...
    println!("✅ Created system user: senagent");
    Ok(())
}

/// Change ownership of a path to senagent:senagent
#[cfg(unix)]
fn chown_to_senagent(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["senagent:senagent", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to senagent:senagent: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_senagent(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_senagent(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "senagent:senagent", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to senagent:senagent: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_senagent(_path: &Path) -> Result<()> {
    Ok(())
}
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(PathBuf::from(fields[5]).join(".senagent"));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'senagent' cannot write {} ({details}). \
             Re-run `sudo senagent service install` and ensure ownership is senagent:senagent.",
            path.display(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/senagent",
            exe_path.display(),
            exe_path.display()
        );
    }
...
command_user="senagent:senagent"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/senagent/access.log"
error_log="/var/log/senagent/error.log"
...
start_pre() {{
    checkpath --directory --owner senagent:senagent --mode 0750 /var/lib/senagent
}}
"#,
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/senagent");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo senagent service install"
        );
    }

    ensure_senagent_user()?;

    let exe = resolve_openrc_executable()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/senagent");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/senagent");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_senagent(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by senagent:senagent)",
    ...
    chown_recursive_to_senagent(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/senagent");
    ...
    run_checked(Command::new("rc-update").args(["add", "senagent", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/senagent");
    println!("   Config path: /etc/senagent/config.toml");
    println!("   Start with: sudo senagent service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("senagent-daemon.cmd");
    let stdout_log = logs_dir.join("daemon.stdout.log");
    let stderr_log = logs_dir.join("daemon.stderr.log");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/senagent.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "SenAgentOS Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/senagent");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/senagent"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"senagent\""));
        assert!(script.contains("description=\"SenAgentOS daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/senagent\""));
        assert!(script.contains("command_args=\"--config-dir /etc/senagent daemon\""));
        assert!(!script.contains("env SENAGENTOS_CONFIG_DIR"));
        assert!(!script.contains("env SENAGENTOS_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"senagent:senagent\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/senagent/access.log\""));
        assert!(script.contains("error_log=\"/var/log/senagent/error.log\""));
    ...
    fn generate_openrc_script_sets_home_for_browser() {
        ...
        assert!(
            script.contains("export HOME=\"/var/lib/senagent\""),
    ...
    fn generate_openrc_script_creates_home_directory() {
        ...
        assert!(
            script.contains("checkpath --directory --owner senagent:senagent"),
            "start_pre must ensure /var/lib/senagent exists with correct ownership"
        );
    }
    ...
    fn systemd_unit_contains_home_and_pass_environment() {
        let unit = "[Unit]\n\
             ...
             [Service]\n\
             Type=simple\n\
             ExecStart=/usr/local/bin/senagent daemon\n\
             Restart=always\n\
             RestartSec=3\n\
             # Ensure HOME is set so headless browsers can create profile/cache dirs.\n\
             Environment=HOME=%h\n\
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/senagent");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/senagent");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/senagent");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/senagent"), true);
    ...
    fn detect_homebrew_var_dir_from_cellar_path() {
        let exe = PathBuf::from("/opt/homebrew/Cellar/senagent/1.2.3/bin/senagent");
        let var_dir = detect_homebrew_var_dir(&exe);
        assert_eq!(var_dir, Some(PathBuf::from("/opt/homebrew/var/senagent")));
    }

    #[test]
    fn detect_homebrew_var_dir_intel_cellar_path() {
        let exe = PathBuf::from("/usr/local/Cellar/senagent/1.0.0/bin/senagent");
        let var_dir = detect_homebrew_var_dir(&exe);
        assert_eq!(var_dir, Some(PathBuf::from("/usr/local/var/senagent")));
    }

    #[test]
    fn detect_homebrew_var_dir_non_homebrew_path() {
        let exe = PathBuf::from("/home/user/.cargo/bin/senagent");
        let var_dir = detect_homebrew_var_dir(&exe);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/senagent/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/senagent/workspace'".to_string(),
                "senagent".to_string()
            ]
        );
    }
    ...
    fn tail_file_errors_on_missing_file() {
        let missing = Path::new("/tmp/senagent-test-nonexistent-log-file.log");
