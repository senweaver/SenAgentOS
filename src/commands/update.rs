...
//! `senagent update` — self-update pipeline with rollback.
...
const GITHUB_RELEASES_LATEST_URL: &str =
    "https://api.github.com/repos/senweaver/SenAgentOS/releases/latest";
const GITHUB_RELEASES_TAG_URL: &str =
    "https://api.github.com/repos/senweaver/SenAgentOS/releases/tags";
...
pub async fn check(target_version: Option<&str>) -> Result<UpdateInfo> {
    ...
    let client = reqwest::Client::builder()
        .user_agent(format!("senagent/{current}"))
...
pub async fn run(target_version: Option<&str>) -> Result<()> {
    ...
    let download_path = temp_dir.path().join("senagent_new");
...
async fn download_binary(url: &str, dest: &Path) -> Result<()> {
    let client = reqwest::Client::builder()
        .user_agent(format!("senagent/{}", env!("CARGO_PKG_VERSION")))
    ...
    // Release assets are .tar.gz archives containing a single `senagent` binary.
...
/// Extract the `senagent` binary from a `.tar.gz` archive.
fn extract_tar_gz(archive_bytes: &[u8], dest: &Path) -> Result<()> {
    ...
    for entry in archive.entries().context("failed to read tar entries")? {
        ...
        // The archive contains a single binary named "senagent" (or "senagent.exe" on Windows).
        let file_name = path.file_name().and_then(|n| n.to_str()).unwrap_or("");

        if file_name == "senagent" || file_name == "senagent.exe" {
            let mut buf = Vec::new();
            entry
                .read_to_end(&mut buf)
    ...
    bail!("archive does not contain a 'senagent' binary")
}
...
async fn validate_binary(path: &Path) -> Result<()> {
    ...
    if !stdout.contains("senagent") {
        bail!("downloaded binary does not appear to be senagent");
    }
...
mod tests {
    ...
    fn find_asset_url_picks_correct_gnu_over_android() {
        let release = make_release(&[
            "senagent-aarch64-linux-android.tar.gz",
            "senagent-aarch64-unknown-linux-gnu.tar.gz",
            "senagent-x86_64-unknown-linux-gnu.tar.gz",
            "senagent-x86_64-apple-darwin.tar.gz",
            "senagent-aarch64-apple-darwin.tar.gz",
        ]);
    ...
    fn extract_tar_gz_finds_binary() {
        ...
        // Build a tar.gz in memory containing a fake "senagent" binary.
        ...
        let mut tar_buf = Vec::new();
        {
            ...
            header.set_cksum();
            builder
                .append_data(&mut header, "senagent", &fake_binary[..])
                .unwrap();
        ...
        let dest = tmp.path().join("senagent_extracted");
    ...
    fn extract_tar_gz_errors_on_missing_binary() {
        ...
        // Build a tar.gz with a file that is NOT named "senagent".
        let mut tar_buf = Vec::new();
        {
            let mut builder = tar::Builder::new(&mut tar_buf);
            let mut header = tar::Header::new_gnu();
