...
/// - `X-SenAgentOS-Timestamp` — unix epoch seconds
/// - `X-SenAgentOS-Nonce` — random UUID v4
/// - `X-SenAgentOS-Signature` — HMAC-SHA256 hex digest
///
...
impl NodeTransport {
    ...
    pub async fn send(
        ...
        let resp = self
            .http
            .post(&url)
            .header("X-SenAgentOS-Timestamp", timestamp.to_string())
            .header("X-SenAgentOS-Nonce", &nonce)
            .header("X-SenAgentOS-Signature", &signature)
