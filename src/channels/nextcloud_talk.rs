...
impl NextcloudTalkChannel {
    ...
    ///
    /// Prevents feedback loops where SenAgentOS reacts to its own messages.
    fn is_bot_name(&self, name: &str) -> bool {
        let name = name.to_ascii_lowercase();
        // Match the configured bot name, or the known bot name "senagent".
        (!self.bot_name.is_empty() && name == self.bot_name) || name == "senagent"
    }
