...
impl MatrixChannel {
    ...
    pub fn new_with_session_hint_and_senagent_dir(
        homeserver: String,
        access_token: String,
        room_id: String,
        allowed_users: Vec<String>,
    ...
    async fn load_or_generate_device_id(&self) -> anyhow::Result<String> {
        ...
        let device_id = format!("SENAGENTOS_{}", &uuid::Uuid::new_v4().to_string()[..8]);
        tracing::info!(
...
impl Channel for MatrixChannel {
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        ...
        if self.voice_mode.load(Ordering::Relaxed) {
            ...
            let voice_work = std::path::PathBuf::from("/tmp/senagent-voice");
            ...
            async move {
                ...
                let body = if let Some((url, filename)) = media_download {
                    let workspace = std::path::PathBuf::from(
                        shellexpand::tilde(
                            &std::env::var("SENAGENTOS_WORKSPACE")
                                .unwrap_or_else(|_| "/tmp/senagent-uploads".to_string()),
                        )
                        .as_ref(),
                    );
...
mod tests {
    ...
    #[test]
    fn matrix_store_dir_is_derived_from_senagent_dir() {
        let ch = MatrixChannel::new_with_session_hint_and_senagent_dir(
            "https://matrix.org".to_string(),
            "tok".to_string(),
            "!r:m".to_string(),
            vec![],
            None,
            None,
            Some(PathBuf::from("/tmp/senagent")),
        );

        assert_eq!(
            ch.matrix_store_dir(),
            Some(PathBuf::from("/tmp/senagent/state/matrix"))
        );
    }

    #[test]
    fn matrix_store_dir_absent_without_senagent_dir() {
        let ch = MatrixChannel::new_with_session_hint(
            "https://matrix.org".to_string(),
            "tok".to_string(),
            "!r:m".to_string(),
