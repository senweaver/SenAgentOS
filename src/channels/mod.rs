...
//! SenAgentOS to external platforms. Each channel implements the [`Channel`] trait
...
const SYSTEMD_STATUS_ARGS: [&str; 3] = ["--user", "is-active", "senagent.service"];
const SYSTEMD_RESTART_ARGS: [&str; 3] = ["--user", "restart", "senagent.service"];
const OPENRC_STATUS_ARGS: [&str; 2] = ["senagent", "status"];
const OPENRC_RESTART_ARGS: [&str; 2] = ["senagent", "restart"];
...
async fn load_runtime_defaults_from_config_file(path: &Path) -> Result<ChannelRuntimeDefaults> {
    ...
    if let Some(senagent_dir) = path.parent() {
        let store = crate::security::SecretStore::new(senagent_dir, parsed.secrets.encrypt);
...
fn build_models_help_response(
    ...
    if cached_models.is_empty() {
        let _ = writeln!(
            response,
            "\nNo cached model list found for `{}`. Ask the operator to run `senagent models refresh --provider {}`.",
...
const BLOCK_KIT_PREFIX: &str = "__SENAGENTOS_BLOCK_KIT__";
...
fn build_config_block_kit(
    ...
    let mut provider_select = serde_json::json!({
        "type": "static_select",
        "action_id": "senagent_config_provider",
    ...
    let mut model_select = serde_json::json!({
        "type": "static_select",
        "action_id": "senagent_config_model",
    ...
    let response = match command {
        ...
        ChannelRuntimeCommand::ShowConfig => {
            if msg.channel == "slack" {
                ...
                format!("__SENAGENTOS_BLOCK_KIT__{blocks_json}")
            } else {
...
pub fn build_system_prompt_with_mode_and_autonomy(
    ...
    if prompt.is_empty() {
        "You are SenAgentOS, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct."
            .to_string()
    } else {
        prompt
    }
...
async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels_config.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `senagent onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `senagent daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `senagent service stop && senagent service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        let plist = home
            .join("Library")
            .join("LaunchAgents")
            .join("com.senagent.daemon.plist");
        if !plist.exists() {
            return Ok(false);
        }
        ...
        if !listed.contains("com.senagent.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.senagent.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.senagent.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let openrc_init_script = PathBuf::from("/etc/init.d/senagent");
...
pub(crate) async fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: senagent channel start");
            println!("To check health:    senagent channel doctor");
            println!("To configure:      senagent onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `senagent onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.senagent/config.toml directly");
        }
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `senagent onboard` first.");
        return Ok(());
    }

    println!("🩺 SenAgentOS Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `senagent gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let provider_runtime_options = providers::ProviderRuntimeOptions {
        ...
        senagent_dir: config.config_path.parent().map(std::path::PathBuf::from),
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `senagent onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 SenAgentOS Channel Server");
    println!("  🤖 Model:    {model}");
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn process_channel_message_uses_runtime_default_model_from_store() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            reliability: Arc::new(crate::config::ReliabilityConfig::default()),
            provider_runtime_options: providers::ProviderRuntimeOptions {
                senagent_dir: Some(temp.path().to_path_buf()),
    ...
    fn prompt_injects_workspace_files() {
        ...
        assert!(
            prompt.contains("Name: SenAgentOS"),
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from SenAgentOS 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
...
This is an example JSON object for profile settings."#;
    ...
    fn maybe_restart_daemon_systemd_args_regression() {
        assert_eq!(
            SYSTEMD_STATUS_ARGS,
            ["--user", "is-active", "senagent.service"]
        );
        assert_eq!(
            SYSTEMD_RESTART_ARGS,
            ["--user", "restart", "senagent.service"]
        );
    }

    #[test]
    fn maybe_restart_daemon_openrc_args_regression() {
        assert_eq!(OPENRC_STATUS_ARGS, ["senagent", "status"]);
        assert_eq!(OPENRC_RESTART_ARGS, ["senagent", "restart"]);
    }
    ...
    async fn e2e_photo_attachment_rejected_by_non_vision_provider() {
        ...
        process_channel_message(
            ...
            traits::ChannelMessage {
                ...
                sender: "senagent_user".to_string(),
    ...
    async fn e2e_failed_vision_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .get("test-channel_chat-photo_senagent_user")
    ...
    async fn e2e_failed_non_retryable_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .get("test-channel_chat-format_senagent_user")
            .expect("history should exist for sender");
        assert_eq!(turns.len(), 2);
        assert_eq!(turns[0].role, "user");
        assert_eq!(turns[0].content, "What is WAL?");
    ...
    fn is_stop_command_matches_with_bot_suffix() {
        assert!(is_stop_command("/stop@senagent_bot"));
    }
