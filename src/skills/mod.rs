...
const OPEN_SKILLS_SYNC_MARKER: &str = ".senagent-open-skills-sync";
...
/// Skills live in `~/.senagent/workspace/skills/<name>/SKILL.md`
    ...
    if scripts_blocked && !allow_scripts {
        ...
        eprintln!(
            "warning: skill '{}' was skipped because it contains script files. \
             Set `skills.allow_scripts = true` in your senagent config to enable it.",
            path.file_name()
...
fn open_skills_enabled_from_sources(
    ...
    if let Some(raw) = env_override {
        ...
        if !raw.trim().is_empty() {
            tracing::warn!(
                "Ignoring invalid SENAGENTOS_OPEN_SKILLS_ENABLED (valid: 1|0|true|false|yes|no|on|off)"
            );
        }
    }
...
fn open_skills_enabled(config_open_skills_enabled: Option<bool>) -> bool {
    let env_override = std::env::var("SENAGENTOS_OPEN_SKILLS_ENABLED").ok();
...
fn resolve_open_skills_dir(config_open_skills_dir: Option<&str>) -> Option<PathBuf> {
    let env_dir = std::env::var("SENAGENTOS_OPEN_SKILLS_DIR").ok();
    ...
    if !readme.exists() {
        std::fs::write(
            ...
            "# SenAgentOS Skills\n\n\
             ...
             ```bash\n\
             senagent skills install <source>\n\
             senagent skills list\n\
             ```\n",
        )?;
    }
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.senagent");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.senagent/skills"));
    }
    ...
    fn load_skills_with_config_reads_open_skills_dir_without_network() {
        ...
        let _enabled_guard = EnvVarGuard::unset("SENAGENTOS_OPEN_SKILLS_ENABLED");
        let _dir_guard = EnvVarGuard::unset("SENAGENTOS_OPEN_SKILLS_DIR");
