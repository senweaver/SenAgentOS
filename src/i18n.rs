...
///
/// Checks `SENAGENTOS_LOCALE`, then `LANG`, then `LC_ALL`.
/// Returns "en" if none are set or parseable.
pub fn detect_locale() -> String {
    if let Ok(val) = std::env::var("SENAGENTOS_LOCALE") {
...
"#,
    ...
    fn detect_locale_from_env() {
        ...
        let saved = std::env::var("SENAGENTOS_LOCALE").ok();
        let saved_lang = std::env::var("LANG").ok();

        crate::util::set_env_var("SENAGENTOS_LOCALE", "ja-JP");
        assert_eq!(detect_locale(), "ja-JP");

        crate::util::remove_env_var("SENAGENTOS_LOCALE");
        crate::util::set_env_var("LANG", "zh_CN.UTF-8");
        assert_eq!(detect_locale(), "zh-CN");

        // Restore.
        match saved {
            Some(v) => crate::util::set_env_var("SENAGENTOS_LOCALE", v),
            None => crate::util::remove_env_var("SENAGENTOS_LOCALE"),
        }
        match saved_lang {
