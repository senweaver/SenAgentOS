...
//! Flash SenAgentOS Arduino firmware via arduino-cli.
//!
...
/// SenAgentOS Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
...
/// Flash SenAgentOS firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("senagent_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling SenAgentOS Arduino firmware...");
    ...
    println!("SenAgentOS firmware flashed successfully.");
