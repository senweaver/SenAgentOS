...
//! Deploy SenAgentOS Bridge app to Arduino Uno Q.
...
pub fn setup_uno_q_bridge(host: Option<&str>) -> Result<()> {
    ...
    if let Some(h) = host {
        if bridge_dir.exists() {
            ...
            anyhow::bail!(
                "Bridge app not found at {}. Run from senagent repo root.",
                bridge_dir.display()
            );
        }
    } else {
...
fn deploy_remote(host: &str, bridge_dir: &std::path::Path) -> Result<()> {
    ...
    println!("SenAgentOS Bridge app started. Add to config.toml:");
...
fn deploy_local(bridge_dir: Option<&std::path::Path>) -> Result<()> {
    ...
    println!("SenAgentOS Bridge app started.");
    Ok(())
}
