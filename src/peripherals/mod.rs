...
/// Handle `senagent peripheral` subcommands.
    ...
    match cmd {
        crate::PeripheralCommands::List => {
            ...
            if boards.is_empty() {
                ...
                println!();
                println!("Add one with: senagent peripheral add <board> <path>");
                println!("  Example: senagent peripheral add nucleo-f401re /dev/ttyACM0");
                println!();
