...
fn ensure_dashboard_assets(_dist_dir: &Path) {
    // The Rust gateway serves `web/dist/` via rust-embed under `/_app/*`.
    // Dashboard assets are expected in web/dist/ - no external assets to copy.
    // Users can build their own web dashboard using `cd web && npm install && npm run build`.
}
